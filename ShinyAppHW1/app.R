...
#
# AI Disclosure: Portions of this application were developed with the
# assistance of generative AI tools (Claude by Anthropic) for code
# structure, debugging, and formatting suggestions. All code was
...
ui <- page_navbar(
  ...
  nav_panel(
    ...
    div(
      ...
      p("U.S. county-level estimates of COVID-19 vaccine hesitancy paired with the",
        ...
        "states / territories."),
      p("Data source: ",
        tags$a(href = "https://data.cdc.gov/Vaccinations/Vaccine-Hesitancy-for-COVID-19-County-and-local-es/q9mh-h2tw/about_data",
               target = "_blank",
               "CDC Vaccine Hesitancy for COVID-19 \u2014 County and Local Estimates")),
      h4("How to use"),
      tags$ol(
      ...
      hr(),
      h4("About this app"),
      p(strong("Created by Sowmya Srinivasan"), " \u2014 VTPEH 6270, Spring 2026"),
      p(tags$a(href = "https://github.com/SS0908/VTPEH-6270-R-Lab",
               target = "_blank",
               icon("github"), " View source on GitHub")),
      p(em("AI Disclosure: Portions of this application were developed with the
            assistance of generative AI tools (Claude by Anthropic) for code
            structure, debugging, and formatting suggestions. All code was
            reviewed and tested by the author."))
    )
  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        width = 320,
        title = "Browse",
        selectInput("data_state", "State filter:",
                    choices = state_choices, selected = "ALL"),
        selectInput("data_svi", "SVI Category:",
                    choices = c("All", levels(hesitancy$svi_cat)),
                    selected = "All"),
        numericInput("data_page", "Page:", value = 1, min = 1, step = 1),
        sliderInput("data_pagesize", "Rows per page:",
                    min = 10, max = 100, value = 25, step = 5),
        hr(),
        helpText(textOutput("data_pageinfo", inline = TRUE))
      ),
      card(
  ...
  nav_spacer(),
  nav_item(
    tags$a("Data source: CDC Vaccine Hesitancy",
           href = "https://data.cdc.gov/Vaccinations/Vaccine-Hesitancy-for-COVID-19-County-and-local-es/q9mh-h2tw/about_data",
           target = "_blank",
           style = "color: white;")
  ),
  nav_item(
    tags$a(icon("github"), " GitHub",
           href = "https://github.com/SS0908/VTPEH-6270-R-Lab",
           target = "_blank",
           style = "color: white;")
  )
)
...
server <- function(input, output, session) {
  ...
  data_filtered <- reactive({
    df <- filter_state(hesitancy, input$data_state)
    if (!is.null(input$data_svi) && input$data_svi != "All") {
      df <- df %>% filter(svi_cat == input$data_svi)
    }
    df %>%
      transmute(
        FIPS    = fips,
        County  = county,
        State   = state,
        `Est. hesitant (%)`           = round(est_hesitant, 2),
        `Est. hesitant or unsure (%)` = round(est_hes_unsure, 2),
        `Est. strongly hesitant (%)`  = round(est_strong_hes, 2),
        SVI                           = round(svi, 3),
        `SVI Category`                = as.character(svi_cat),
        `% fully vaccinated`          = round(pct_vax, 2)
      )
  })
  
  output$data_pageinfo <- renderText({
    df <- data_filtered()
    n  <- nrow(df)
    pages <- max(1, ceiling(n / input$data_pagesize))
    sprintf("%d rows \u00b7 %d page(s) at %d/page",
            n, pages, input$data_pagesize)
  })
  
  output$data_table <- renderTable({
    df <- data_filtered()
    n  <- nrow(df)
    if (n == 0) return(data.frame(Message = "No rows match the current filters."))
    
    pagesize <- input$data_pagesize
    pages    <- max(1, ceiling(n / pagesize))
    page     <- min(max(1, input$data_page), pages)
    
    start <- (page - 1) * pagesize + 1
    end   <- min(page * pagesize, n)
